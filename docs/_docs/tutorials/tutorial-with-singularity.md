...
- Find the absolute path to where you saved the host human genome file (GRCh38_latest_genome.fna)
- In the command below replace the `<path/to/human/GRCh38_latest_genome.fna>` with the path from above
...
[*TIP:* these commands can get really long, one way is to use `vim` editor or open up notepad and copy paste the commands, change the paths and then copy and paste to the terminal. If you use this method then you will need to keep the backslashes (`\`) at the end of the line]
...
- Find the absolute path of where your Kraken2 reference database is located
- In the command below, replace the `<path/to/Kraken2_db>` with the absolute path
...
- Find the absolute paths for the 4 reference databases and like before replace these in the below command
