...
<tr>
  ...
  <td rowspan=3><img src="{{site.baseurl}}/assets/img/tutorials/no-singularity/tut_OverallSchema.png"/></td>
</tr>
...
<tr>
  ...
  <td rowspan=5><img src="{{site.baseurl}}/assets/img/tutorials/no-singularity/tut_QCpipeline.png"/></td>
</tr>
...
<tr><td>5. (optional) QC\_report generates a summary report *after all* PBS scripts have been run for all samples in the study</td></tr>
</table>
...
| <div style="width:180px">Parameters</div> | Required? | Description |
| ---------- | ----------|----------- |
...
- The first 13 lines are the same as `pbs_header_taxa.cfg` configuration file as it has been directly inserted (see above [PBS configuration files](#pbs-configuration-files) for explanation)
...
<td rowspan=2><img src="{{site.baseurl}}/assets/img/tutorials/no-singularity/tut_function_pipeline.png"/></td>
</tr>
...
- The first few lines come from the `pbs_header_func.cfg` configuration file (see [PBS configuration files](#pbs-configuration-files))
  - note that the walltime is set to 8 hours, increase as needed
