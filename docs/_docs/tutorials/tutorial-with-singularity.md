...
<td>
<ol>
<li>**Quality control** (QC) of the sequenced reads</li>
<li>**Taxonomy profiling** after QC, for assigning reads to taxon (this step can be run in parallel with step 3)</li>
<li>**Functional profiling** after QC, for assigning reads to genes (this step can be run in parallel with step 2)</li>
</ol></td></tr>
</table>
...
*pbs_header_qc.cfg* is a configuration file containing PBS and Singularity settings. This be copied to the top of all PBS script files. Below is the PBS configuration file we have provided for you.
...
- The first 13 lines are the same as `pbs_header_qc.cfg` configuration file as it has been directly inserted (see above [PBS configuration files](#pbs-configuration-files) for explanation)
...
Since Kraken requires big memory (~300GB), there is only one PBS script that will execute each sample sequentially.
...
2. Bracken takes the Kraken2 output to estimate abundances for a given taxonomic rank. This is repeated from Phylum to Species level.
3. The final step (**generate table**) is performed after *all samples* have been processed. This combines the output and generates a *feature table* for a given taxonomic rank. The feature table contains discrete counts and relative abundances of *taxon X occurring in sample Y*.
...
- the first 13 lines are directly copied from the `pbs_header.cfg` file
